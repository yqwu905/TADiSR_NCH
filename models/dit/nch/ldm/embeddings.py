...
def get_2d_sincos_pos_embed(
    ...
    grid_h = np.arange(grid_size[0], dtype=torch.float32) / (grid_size[0] / base_size) / interpolation_scale
    grid_w = np.arange(grid_size[1], dtype=torch.float32) / (grid_size[1] / base_size) / interpolation_scale
    grid = np.meshgrid(grid_w, grid_h)  # here w goes first
    grid = np.stack(grid, axis=0)
...
def apply_rotary_emb(
    ...
    if use_real:
        ...
        out = (x.to(torch.float16) * cos.to(torch.float16) + x_rotated.to(torch.float16) * sin.to(torch.float16)).to(x.dtype)
