...
from .attention_processor_v3_split import (
    Attention,
    AttentionProcessor,
    NCHAttnProcessor2_0,
    SparseAttnProcessor,
...
from .embeddings import CombinedTimestepEmbeddings, QwenEmbedRope
from .npu_utils import fast_interleave
from diffusers.models.modeling_outputs import Transformer2DModelOutput

from einops import rearrange, repeat
from typing import Union, List
from .bmm import SparseProcessAttnAigc, SparseProcessAttnAigc_Local0408
import math
...
class NCHTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_attention_heads, attention_head_dim, qk_norm="rms_norm", eps=1e-6,
                 processor_type='default', sampling=None, sr_ratio=1, mmlp_ratio=4, ffn_type='mlp', attn_type='default', 
                                 adaln_dim=256):
        super().__init__()

        self.norm1 = AdaLayerNormZero(dim, input_dim=adaln_dim)
        self.norm1_context = AdaLayerNormZero(dim, input_dim=adaln_dim)
...
class NCHTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        self.norm_out = AdaLayerNormContinuous(self.inner_dim, self.adaln_dim, elementwise_affine=False, eps=1e-6)
        self.proj_out = nn.Linear(self.inner_dim, patch_size * patch_size * self.out_channels, bias=True)

        self.norm_out_60 = AdaLayerNormContinuous(self.inner_dim, self.adaln_dim, elementwise_affine=False, eps=1e-6)
        self.proj_out_60 = nn.Linear(self.inner_dim, patch_size * patch_size * self.out_channels, bias=True)

        self.norm_out_40 = AdaLayerNormContinuous(self.inner_dim, self.adaln_dim, elementwise_affine=False, eps=1e-6)
        self.proj_out_40 = nn.Linear(self.inner_dim, patch_size * patch_size * self.out_channels, bias=True)

        self.norm_out_30 = AdaLayerNormContinuous(self.inner_dim, self.adaln_dim, elementwise_affine=False, eps=1e-6)
    ...
    def rearrange(self, hidden_states, bs, c, h, w, patch_size):
        hidden_states = hidden_states.reshape(bs, c, h // patch_size, patch_size, w // patch_size, patch_size)
        hidden_states = hidden_states.reshape(bs * c * h // patch_size, patch_size, w // patch_size, patch_size)
        hidden_states = hidden_states.permute(0, 2, 1, 3).reshape(bs, c, h // patch_size * w // patch_size, patch_size * patch_size)
        hidden_states = hidden_states.permute(0, 2, 1, 3).reshape(bs, h // patch_size, w // patch_size, c, patch_size, patch_size)
    ...
    def reshape1D(self, hidden_states, new_H, new_W, block_lenth_2D, new_img_len):
        bs = hidden_states.shape[0]
        return hidden_states.view(bs*new_H, block_lenth_2D, new_W, -1).transpose(1, 2).reshape(bs, new_img_len, -1)
    
    def unreshape1D(self, hidden_states, new_H, new_W, block_lenth_2D, new_img_len):
        bs = hidden_states.shape[0]
        return hidden_states.reshape(bs*new_H, new_W, block_lenth_2D, -1).transpose(1, 2).reshape(bs, new_img_len, -1)
    
    def convert_rope(self, image_rotary_emb, new_H, new_W, block_lenth_2D, new_img_len, dtype, device):
        freqs_cis_img, freqs_cis_txt = image_rotary_emb
        ## todo 

        cos_img = freqs_cis_img.real
        sin_img = freqs_cis_img.imag


        # 常见布局 [B, S, H, D//2] -> cos 需变为 [1, S, 1, D//2]
        cos_img = cos_img.unsqueeze(0).unsqueeze(2)
        sin_img = sin_img.unsqueeze(0).unsqueeze(2)


        cos_img = fast_interleave(cos_img, 2, dim=-1)
        sin_img = fast_interleave(sin_img, 2, dim=-1)

        cos_txt = freqs_cis_txt.real
        sin_txt = freqs_cis_txt.imag


        cos_txt = cos_txt.unsqueeze(0).unsqueeze(2)
        sin_txt = sin_txt.unsqueeze(0).unsqueeze(2)


        cos_txt = fast_interleave(cos_txt, 2, dim=-1)
        sin_txt = fast_interleave(sin_txt, 2, dim=-1)


        cos_img = cos_img.reshape(new_H, block_lenth_2D, new_W, -1).permute(0, 2, 1, 3).contiguous().view(1, new_img_len, 1, -1)
        sin_img = sin_img.reshape(new_H, block_lenth_2D, new_W, -1).permute(0, 2, 1, 3).contiguous().view(1, new_img_len, 1, -1)

        new_cos = torch.cat([cos_txt, cos_img], dim=1).transpose(1, 2).contiguous().to(dtype=dtype, device=device)
        new_sin = torch.cat([sin_txt, sin_img], dim=1).transpose(1, 2).contiguous().to(dtype=dtype, device=device)
    ...
    def forward(
        ...
        hidden_states = self.reshape1D(hidden_states_new, new_H, new_W, block_lenth_2D, new_img_len)
        ...
        if enable_skip_level == "100":
            hidden_states = self.norm_out(hidden_states, temb)
            output = self.proj_out(hidden_states)
        elif enable_skip_level == "60":
            hidden_states = self.norm_out_60(hidden_states, temb)
            output = self.proj_out_60(hidden_states)
        elif enable_skip_level == "40":
            hidden_states = self.norm_out_40(hidden_states, temb)
            output = self.proj_out_40(hidden_states)
        elif enable_skip_level == "30":
            hidden_states = self.norm_out_30(hidden_states, temb)
            output = self.proj_out_30(hidden_states)
        else:
            raise ValueError(f"Invalid enable_skip_level: {enable_skip_level}!")
