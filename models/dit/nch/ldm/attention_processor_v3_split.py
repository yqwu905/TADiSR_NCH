...
def apply_rotary_emb_qwen_ruduan(
    x: torch.Tensor,
    new_cos: torch.Tensor,
    new_sin: torch.Tensor,
) -> Tuple[torch.Tensor, torch.Tensor]:
    x_real, x_imag = x.reshape(*x.shape[:-1], -1, 2).unbind(-1)  # [B, S, H, D//2]
    x_rotated = torch.stack([-x_imag, x_real], dim=-1).flatten(3)
    out = (x * new_cos + x_rotated * new_sin)
    return out
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.sampling = sampling  # ['conv', 'avg_pool']
        self.sr_ratio = sr_ratio
        if sr_ratio > 1 and sampling == 'conv':
            # Avg Conv Init.
            self.sr = nn.Conv2d(out_dim, out_dim, groups=out_dim, kernel_size=sr_ratio, stride=sr_ratio)
...
class NCHAttnProcessor2_0:
    ...
    def __call__(
        ...
        if image_rotary_emb is not None:
            new_cos, new_sin = image_rotary_emb
...
class SparseAttnProcessor:
    ...
    def __init__(self):
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError(
                "SparseAttnProcessor requires PyTorch 2.0, to use it, please upgrade PyTorch to 2.0."
            )
